...
class TestConnSpec:
    ...
    def test_tf_decomp(self, ):
        ...
        for mode in ["morlet", "multitaper"]:
            ...
            actual2 = out.mean(
                axis=(0, -1))[:, self.__get_freqs_indexes(28, 32)].mean(1)
            np.testing.assert_allclose(
                actual1, val*np.ones_like(actual), atol=atol)
            np.testing.assert_allclose(
                actual2, val*np.ones_like(actual), atol=atol)
    ...
    def test_conn_spec(self,):
        ...
        for method in ['coh', 'plv']:
            ...
            actual_1 = out.mean("trials").sel(freqs=slice(
                8, 12), times=slice(0.5, 2.2)).mean(dim=("freqs"))
            actual_2 = out.mean("trials").sel(freqs=slice(
                28, 33), times=slice(2.8, 4.7)).mean(dim=("freqs"))
            if method == "coh":
                val = 0.8
            else:
                val = 0.9
            np.testing.assert_allclose(
                actual_1, val*np.ones_like(actual_1), atol=0.1)
            np.testing.assert_allclose(
                actual_2, val*np.ones_like(actual_2), atol=0.1)
